...
> [!NOTE]
> Изначально проект создавался для старых версий 3x-ui до v2.9.0, где не было нативных Clash-подписок и агрегации нескольких серверов.
> В новых версиях этот проект может быть нужен не всем, в зависимости от ваших потребностей.

Поддерживаемые форматы вывода:

- VLESS/base64-подписка
- Clash/Mihomo YAML, сгенерированный из VLESS-подписок
- Нативная Clash/Mihomo YAML-подписка
...
- [Эндпоинты](#эндпоинты)
- [Объяснение настройки URL](#объяснение-настройки-url)
- [Справочник по конфигурации](#справочник-по-конфигурации)
- [Переменные окружения](#переменные-окружения)
- [Файл списка источников](#файл-списка-источников)
- [Файлы шаблонов Clash](#файлы-шаблонов-clash)
- [Локальный запуск (без Docker)](#локальный-запуск-без-docker)
- [Устранение неполадок](#устранение-неполадок)
- [Указание авторства и лицензия](#указание-авторства-и-лицензия)
...
- автоматически группирует прокси по первому слову очищенного имени прокси, если нет ручного файла групп,
...
```bash
git clone https://github.com/darkzoul5/3x-ui_sub_aggregator.git
cd xray_config_aggregator
```

2. Подготовьте локальную директорию конфигов и список источников:
...
3. Отредактируйте `configs/config.txt` и укажите по одному URL источника на строку:
...
4. Настройте переменные окружения в `docker-compose.yml` (или в своем `.env`-файле).
...
Если `proxy-groups-{sub_id}.yaml` существует, он полностью заменяет автогенерацию.
Иначе приложение автоматически генерирует группы из возвращённых Clash-прокси.
Если `proxy-groups-{sub_id}.yaml` отсутствует, используется `default-proxy-groups.yaml`, если он есть.

Автогенерация групп для Clash работает так:

- имя прокси сначала очищается только от хвостового email-подобного суффикса;
- имя самого прокси при этом не изменяется;
- имя группы берется из первого слова очищенного имени;
- если в группе только один прокси, это все равно остается валидной группой;
- если есть `proxy-groups-{sub_id}.yaml`, он полностью заменяет автогенерацию.
